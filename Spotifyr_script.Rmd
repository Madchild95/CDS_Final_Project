...
my_plist <- get_user_playlists(my_id)
husk <- my_plist %>%
  filter(name %in% 'Husk')
#extracting tracks data from playlist
tracks_on_husk <- get_playlist_tracks(husk$id) %>% 
  mutate(artist.name = map_chr(track.artists, function(x) x$name[1])) %>% 
  select(track.name, track.popularity, artist.name, track.album.name, track.id, track.uri) %>% 
  rename(uri = track.uri)
#extract features
features_on_husk <- get_track_audio_features(tracks_on_husk$track.id)

#merge the two columns
husk_merged <- merge(tracks_on_husk, features_on_husk, by = 'uri')

#apply function
sentiment_profiles <- husk_merged %>% 
        rowwise %>% 
...
tracks <- get_playlist_tracks(sahara$id)
